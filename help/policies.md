...
> **Note:** Policies are separate from permissions. Permissions control what tools can do during a session, while policies control whether OpenCode may use a resource such as an LLM provider.
...
### Example: Deny a provider

Deny use of the `openai` provider:
...
> **Note:** More policy actions may be added in the future.

---

## Matching

The `resource` field supports wildcard matching:

- `*` — Match zero or more characters.
- `?` — Match exactly one character.

### Example: Wildcard denial
...
### Replacing `enabled_providers`

Deny all providers first, then allow the selected providers after it:
