...
2. Delegate to `@explore` / `@scout` / `@general` only when broad codebase context is required. Follow `subagent-reuse`.
...
### 3. Implement & Verify
1. Partition work into `TaskUnit`s with NON-overlapping file sets; dispatch parallel `@general` (max 2 slots `ctx-3..4`). Follow `subagent-reuse` to track subagent IDs and wait for ALL before finishing. If two units must touch the same file, merge them into one unit or run them sequentially.
...
- Slot Limits: `explore ≤ 1`, `scout ≤ 1`, `general ≤ 2`.
