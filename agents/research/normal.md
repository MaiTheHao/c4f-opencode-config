...
2. Take the full response text from Stage 4 (Synthesis). Launch `general` with: "Replace {timestamp} with current date/time, replace {topic-slug} with a short slug from the topic. Prepend a header '# Research Output\n\nDate: {timestamp}\n\n' to this content. Create research-outputs/ if not exists. Save to the resulting path:\n\n[full response text]"
